...
def _sub(path: str, key: str) -> str:
    ...
    FLIKGUIDEN och Home ritar. Etiketten finns (skärmläsare) men döljs."""
    return st.radio(f"Val i {path.replace('/', ' → ')}", nav.options(path),
                    label_visibility="collapsed", horizontal=True, key=key)
...
def main():
    ...
    tab_labels = [f"  {label}  " for _k, label in nav.TOP]
    ...
    if _is_open(tabs["screening"]):
        with tabs["screening"]:
            sub = _sub("screening", "sub_screening")
            st.markdown("---")
            if sub == "Arc Screener":
                inner = _sub("screening/Arc Screener", "sub_screening_arc")
                if inner == "Wolf":
                    tab_screener()
                elif inner == "Viking":
                ...
                inner = _sub("screening/Contrarian Alpha", "sub_screening_contrarian")
                if inner == "Screener":
    ...
    if _is_open(tabs["review"]):
        with tabs["review"]:
            ...
            if sub in ("Rick Rule", "Royalty C"):
                ...
                inner = _sub("review/🧭 Durrett & Confidence", "sub_review_durrett")
    ...
    if _is_open(tabs["regime"]):
        with tabs["regime"]:
            group = _sub("regime", "sub_regime_group")
            if group == "Marknad":
                sub = _sub("regime/Marknad", "sub_regime")
                st.markdown("---")
                if sub == "Arc Regime":
                    inner = _sub("regime/Marknad/Arc Regime", "sub_regime_arc")
                    ...
                    inner = _sub("regime/Marknad/Alpha Regime", "sub_regime_alpha")
                ...
                sub = _sub("regime/Råvaror", "sub_regime_commodities")
                st.markdown("---")
                if sub == "🌍 EMBER Regime":
    ...
    if _is_open(tabs["portfolio"]):
        with tabs["portfolio"]:
            sub = _sub("portfolio", "sub_portfolio")
            st.markdown("---")
    ...
    if _is_open(tabs["rules"]):
        with tabs["rules"]:
            sub_rules = _sub("rules", "sub_rules")
            st.markdown("---")
